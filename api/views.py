...
class ChatView(views.APIView):
    ...
    def post(self, request, slug):
        ...
        convo = (
            get_object_or_404(Conversation, id=convo_id, user=request.user)
            if convo_id else Conversation.objects.create(user=request.user, advisor=advisor)
        )
        Message.objects.create(conversation=convo, role="user", content=request.data["message"])
        ...
        return Response(
            {"conversation_id":convo.id,"reply":bot_text,"messages":MessageSerializer(convo.messages.all(),many=True).data},
